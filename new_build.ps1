...
function Get-RepoRoot {
    if ((Test-Path (Join-Path $PSScriptRoot "packer")) -and (Test-Path (Join-Path $PSScriptRoot "Vagrantfile"))) {
        return $PSScriptRoot
    }

    if ((Test-Path (Join-Path $PSScriptRoot "metasploitable3\packer")) -and (Test-Path (Join-Path $PSScriptRoot "metasploitable3\Vagrantfile"))) {
        return (Join-Path $PSScriptRoot "metasploitable3")
    }

    throw "Could not find the metasploitable3 repository folder."
}
...
function Cleanup-BuildArtifacts {
    ...
    $paths = @(
        (Join-Path $RepoRoot "output-virtualbox-iso"),
        (Join-Path $RepoRoot ".vagrant")
    )
...
function Install-Box($osFull, $osShort) {
    $templatePath = Join-Path $RepoRoot "packer\templates\$osFull.json"
    if (-not (Test-Path $templatePath)) {
        throw "Template not found: $templatePath"
    }

    $boxVersion = Get-BoxVersion $templatePath
    $boxPath = Join-Path $RepoRoot "packer\builds\$($osFull)_virtualbox_$boxVersion.box"
...
function Show-Menu {
    Show-Header "Metasploitable3 Student Build Kit"
...
try {
    Show-Header "Starting Metasploitable3 Student Build Kit"
    Write-Host "Repository root detected as: $RepoRoot" -ForegroundColor Green
    Run-Interactive
}
catch {
    Write-Host ""
